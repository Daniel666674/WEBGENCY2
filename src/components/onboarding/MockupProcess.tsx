import { Sparkles, MessageCircle, AlertCircle, Image, Camera, Video, Tag, Palette, Share2, UserPlus, Upload, Presentation, Calculator, CheckCircle2 } from "lucide-react";
import { Lead, SectionTitle, SubTitle, Callout, InfoCard, FlowSteps } from "./shared";
...
export function MockupProcess() {
  return (
    <div className="space-y-6 max-w-5xl">
      ...
      <Callout tone="primary" icon={Sparkles} title="Por qué funciona">
        <p>
      ...
      <div>
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          <InfoCard title="Logo" icon={Image} color="purple">
            Su logo en la mejor calidad que tengan. Si no tienen o está feo, lo recreamos — pero pedirlo
            abre la conversación.
          </InfoCard>
          <InfoCard title="Fotos de sus redes" icon={Camera} color="blue">
          ...
          <InfoCard title="Videos y reels" icon={Video} color="red">
      ...
      <Callout tone="green" icon={MessageCircle} title="Cómo pedirlo sin fricción">
        <p>
      ...
      <div>
        ...
        <FlowSteps
          items={[
            { title: "Crear el contacto", icon: UserPlus, body: "Se agrega el prospecto en Contactos con su fuente (Instagram, referido, etc.) y sus notas." },
            { title: "Construir y subir el mockup", icon: Upload, body: "Se arma la muestra y se guarda su enlace (mockupUrl) en el contacto. El dashboard resalta los mockups listos sin enviar." },
            { title: "Presentarlo y crear el deal", icon: Presentation, body: "Se le muestra al cliente y se crea un Deal en el Pipeline, moviéndolo a la etapa 'Propuesta'." },
            { title: "Cotizar con la Calculadora", icon: Calculator, body: "Se arma el precio real con plan base + módulos + mensualidad, y se genera una Propuesta." },
            { title: "Cerrar y activar", icon: CheckCircle2, body: "Al aceptar, el deal pasa a 'Cerrado Ganado' y el contacto se vuelve Cliente Activo con su mensualidad." },
          ]}
        />
      </div>

      <Callout tone="amber" icon={AlertCircle} title="Regla">
        <p>
